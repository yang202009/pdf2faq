---
name: pdf2faq
description: Use when the user wants to deeply understand a PDF, book, report, or any long document — or to turn it into a structured learning FAQ with evidence labeling and source traceability. Trigger on: "帮我读这本书/报告", "生成学习FAQ", "深度理解这份文档", "把这个PDF转成问答", "帮我整理这篇报告的核心", "分析这份文档", "帮我真正理解", "提炼这本书的框架", or any request to extract structured learning value from a document. Also trigger when the user uploads a PDF or TXT and wants to understand it deeply, even if they don't use the word "FAQ".
---
...
7. Judge:
   ...
   → If anchor quality is **中** or **低**, or if the analysis object is derived TXT/MD:
     Load `references/source-integrity-rules.md` NOW and keep it active for all subsequent generation steps.
...
## Generation modes（生成模式，独立于输出模式）

### 文档生产器
条件：source chain 可靠，OCR 质量高，结构完整
行为：
- 生成完整 FAQ（30–50 问）
- 使用完整证据块
- 提供阅读路线图和概念拆解
...
### 分析器
条件：OCR 弱，锚点不稳定，结构碎片化
行为：
- 缩减 FAQ 数量（10–20 问，聚焦最可靠的内容）
- 每个问题降低引用精度，改用文本锚点
- 在每个回答前加注"受限于提取质量，以下基于可见文本推断"
- 不生成阅读路线图（章节边界不可靠时）
- 在输出开头显著说明分析限制
...
- Did you load source-integrity-rules.md if anchor quality is 中 or 低?
