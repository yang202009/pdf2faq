...
**Section 6 degradation rule（低结构质量时）：**
If chapter structure is unclear, chapter boundaries are ambiguous, or anchor quality is 低, replace Section 6 with the following note instead of fabricating a reading order:

> 受限于文档结构清晰度（章节边界不可靠 / OCR 损坏 / 目录缺失），无法提供可靠的分章节阅读路线图。建议按文本顺序通读，重点留意以下关键词出现的位置：{{keywords}}
